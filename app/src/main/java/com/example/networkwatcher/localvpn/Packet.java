...
public class Packet
{
    ...
    private static int globalIPId = 0;
    ...
    public void updateTCPBuffer(ByteBuffer buffer, byte flags, long sequenceNum, long ackNum, int payloadSize)
    {
        ...
        // Her paket için benzersiz IP Identification (Hocaya gösterilecek kritik düzeltme)
        int ipId = ++globalIPId & 0xFFFF;
        backingBuffer.putShort(4, (short) ipId);
        ip4Header.identificationAndFlagsAndFragmentOffset = (ipId << 16) | (ip4Header.identificationAndFlagsAndFragmentOffset & 0xFFFF);
        ...
        // MSS Clamping: SYN paketlerinde MSS değerini 1400 olarak set et (SSL/Handshake sorunlarını çözer)
        int headerSize = TCP_HEADER_SIZE;
        if ((flags & TCPHeader.SYN) != 0) {
            headerSize += 4;
            backingBuffer.put(IP4_HEADER_SIZE + 20, (byte) 2); // Option Kind: MSS
            backingBuffer.put(IP4_HEADER_SIZE + 21, (byte) 4); // Option Length: 4
            backingBuffer.putShort(IP4_HEADER_SIZE + 22, (short) 1400); // MSS Value: 1400
        }

        byte dataOffset = (byte) ((headerSize / 4) << 4);
        tcpHeader.dataOffsetAndReserved = dataOffset;
        backingBuffer.put(IP4_HEADER_SIZE + 12, dataOffset);

        updateTCPChecksum(payloadSize, headerSize);

        int ip4TotalLength = IP4_HEADER_SIZE + headerSize + payloadSize;
    ...
    private void updateTCPChecksum(int payloadSize, int tcpHeaderSize)
    {
        int sum = 0;
        int tcpLength = tcpHeaderSize + payloadSize;
